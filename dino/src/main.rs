    ...
    loop {
        ...
        if !game_over {
            ...
            for obstacle in obstacles.iter_mut() {
                ...
                draw_rectangle(obstacle.x, obstacle.y, obstacle.w, obstacle.h, GREEN);
                
                let player_rect = Rect::new(50.0, player_y, player_size, player_size);
                if player_rect.overlaps(obstacle) {
                    game_over = true;
                    if score > high_score {
                        high_score = score;
                    }
                ...
                if obstacle.x + obstacle.w < 50.0 && obstacle.x + obstacle.w > 49.0 {
                    score += 1;
                }
            }

            obstacles.retain(|obstacle| obstacle.x + obstacle.w > 0.0);
